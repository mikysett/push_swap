...
void	ft_extract_ranges(t_data *data)
{
	...
	{
		top = data->s_a.size - 1;
		if (data->s_a.stack[top].is_sorted)
		{
			ft_rotate_a(data);
			DEBUG_CODE(data->stats.fill_b++;)
			continue;
		}
		if (data->s_a.stack[top].range == biggest->id)
		{
			ft_push_b(data);
			biggest->nb_ranged_in_b++;
			if (biggest->nb_ranged_in_b == biggest->nb_in_range)
				biggest = ft_get_biggest_range(data);
			DEBUG_CODE(data->stats.fill_b++;)
		}
		else if (data->s_a.stack[top].range == second->id)
		{
			ft_push_b(data);
			ft_rotate_b(data);
			second->nb_ranged_in_b++;
			if (second->nb_ranged_in_b == second->nb_in_range)
				second = ft_get_biggest_range(data);
			DEBUG_CODE(data->stats.fill_b += 2;)
		}
		else
		{
			ft_rotate_a(data);
...
t_range	*ft_get_biggest_range(t_data *data)
{
	...
	{
		if (!data->ranges[i].already_ranged
			&& (!biggest
				|| data->ranges[i].nb_in_range > biggest->nb_in_range))
			biggest = &data->ranges[i];
		i++;
	}
	if (!biggest)
		return (&data->ranges[0]);
	biggest->already_ranged = true;
	return (biggest);
}
